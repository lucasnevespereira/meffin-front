        ...
        <ul class="divide-y divide-gray-200">
            <li v-for="(item, index) in items" :key="index" class="flex justify-between items-center py-4">
                ...
                <div class="text-sm md:text-base">
                    <span v-if="item.is_fixed && item.day_of_month > 0" class="mr-4 hidden sm:inline">Tous les <b>{{ item.day_of_month }}</b></span>
                    <span v-else-if="item.day_of_month > 0" class="hidden sm:inline">Le <b>{{ item.day_of_month }}</b></span>
